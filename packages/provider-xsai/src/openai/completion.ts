...
export function chatCompletionFromResp(
...
export function resolveFirstTextContentFromChatCmpl(
...
export function resolveToolCallsFromCmpl(
  chatCompletion?: ChatCompletionsResponse,
): ToolCall[][] {
  if (!chatCompletion)
    return []
  ...
  return chatCompletion.choices
    .filter(choice => choice.message.tool_calls != null)
    .map(choice => choice.message.tool_calls!)
}

export function resolveFirstToolCallFromCmpl(
  chatCompletion?: ChatCompletionsResponse,
): ToolCall | undefined {
