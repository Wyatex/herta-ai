...
import type {
  ...
  InvokeContext,
  ResolvedToolCall,
  ToolCallFunctionResult,
} from './types'

export function resolvedToolCall<P = any, R = any>(
  toolCall: ToolCall | null | undefined,
  tools: DefinedTool<P, R>[],
): ResolvedToolCall<P, R> | undefined {
  if (toolCall == null)
    return undefined
  ...
  const foundTool = tools.find(
    tool => toolCall.function.name === tool.tool.function.name,
  )
  if (foundTool == null)
    return undefined
...
export async function invokeFunctionWithResolvedToolCall<
  P = any,
  R = any | undefined,
>(
...
export async function invokeFunctionWithTools(
  chatCompletion: ChatCompletion,
  tools: DefinedTool<any, any>[],
  messages: Message[],
): Promise<Array<ToolCallFunctionResult<any, any>>> {
  ...
  for (const choice of chatCompletion.choices) {
    ...
    for (const toolCall of choice.message.tool_calls) {
      ...
      results.push({
        result: await invokeFunctionWithResolvedToolCall<any, any>(
          chatCompletion,
          tc,
          messages,
        ),
