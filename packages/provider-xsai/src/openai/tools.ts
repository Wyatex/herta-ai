...
export async function toolFunction<S extends Schema, P extends Infer<S>>(
  name: string,
  description: string,
  parameters: S,
): Promise<ToolFunction<P>> {
  return {
    type: 'function',
    function: {
      name,
...
export function defineToolFunction<
  F extends (ctx: InvokeContext<P, R>) => R,
  P = Parameters<F>,
  R = ReturnType<F>,
>(
  tool: ToolFunction<P>,
